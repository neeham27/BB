...
CORS(app)
app.config["MONGO_URI"] = "mongodb+srv://rohitrajesh:dxYqxEjoPrxbltR0@cluster0.bql1r.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
mongo = PyMongo(app)
...
def dashboard(emp_id):
    if emp_id:
        session['emp_id'] = int(emp_id)  # Store emp_id in session
        return render_template("dashboard.html", emp_id=int(emp_id))
    else:
        return redirect(url_for('homepage'))  # Redirect to login if emp_id is not valid

@app.route("/reserve", methods=["GET"])
def reserve():
    if 'emp_id' not in session:
        return redirect(url_for('homepage'))  # Ensure the user is logged in
    return render_template("reserve.html", emp_id=session['emp_id'])

@app.route("/confirmation/<emp_id>/<reservation_id>", methods=["GET"])
def confirmation(emp_id, reservation_id):
    return render_template("confirmation.html", emp_id=int(emp_id), reservation_id=int(reservation_id))
...
def get_employee_reservations():
    ...
    reservation_list = [{"id": str(res["_id"]), "emp_id": res["emp_id"], "date": res["date"], "time": res["time"], "Number_of_slots": res["num_slots"], "reservation_id": res["reservation_id"]} for res in reservations]
...
@app.route("/reservation/details/<int:reservation_id>", methods=["GET"])
def get_reservation_details(reservation_id):
    ...
    if reservation:
        ...
        return jsonify({"status": "Reservation not found"}), 404
